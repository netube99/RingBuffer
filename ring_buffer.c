 ...
 * \return 返回缓冲区初始化的结果
 *      \arg RING_BUFFER_ARRAY_SUCCESS: 初始化成功
 *      \arg RING_BUFFER_ARRAY_ERROR: 初始化失败
*/
...
uint8_t Ring_Buffer_Write_Byte(ring_buffer *ring_buffer_handle, uint8_t data)
{
	...
	return RING_BUFFER_ARRAY_SUCCESS ;
}
...
/**
 * \brief 向缓冲区头部读指定长度的数据
 * \param[in] ring_buffer_handle: 缓冲区结构体句柄
 * \param[out] out_addr: 读取的数据保存地址
 * \param[in] read_lenght: 要读取的字节数
 * \return 返回缓冲区头部读指定长度字节的结果
 *      \arg RING_BUFFER_ARRAY_SUCCESS: 读取成功
 *      \arg RING_BUFFER_ARRAY_ERROR: 读取失败
*/
